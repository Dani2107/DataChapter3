...
adultmean<-adultsd<-pupsmean<-pupsd<-IBImean<-IBIsd<-blankmean<-blanksd<-packnomean<-packnosd<-endmean<-endsd<-rep(0,length(testing))

  
#for(l in 1:testing){
repeats<-1000 #number of times to repeat the loop
...
#adultmean[l]<-mean(mean.pck)
#pupsmean[l]<-mean(mean.pups)
#pupsd[l]<-sd(mean.pups)
#adultsd[l]<-sd(mean.pck)
#IBIsd[l]<-sd(mean.IBI)
#IBImean[l]<-mean(mean.IBI)
#blankmean[l]<-mean(mean.blank)
#blanksd[l]<-sd(mean.blank)
#endmean[l]<-mean(end.all.vec, na.rm=TRUE)
#endsd[l]<-sd(end.all.vec, na.rm=TRUE)
#packnomean[l]<-mean(count.all.packs)
#packnosd[l]<-sd(count.all.packs)


proc.time() - ptm

alloutput1000<-data.frame(mean.pck,sd.pack,mean.pups,sd.pups,mean.IBI,sd.IBI,mean.blank,sd.blank)
titles1000<-c("mean pack","sd pack", "mean pups","sd pups", "mean IBI", "sd IBI","mean blank", "sd blank")

names(alloutput1000)<-titles1000

write.csv(alloutput1000,"C:/Users/rabaiotti.d/Documents/alloutput1000")

datameans<-data.frame(mean.pack=integer(),sd.pack=integer(),mean.pups=integer(),sd.pups=integer(),mean.IBI=integer(),sd.IBI=integer(),mean.blank=integer(),sd.blank=integer())


for(j in 1:8){
  a<-j
  for(i in 1:1000){
    datameans[i,j]<-mean(alloutput1000[1:i,j])
  }
}
...
write.csv(datameans,"C:/Users/rabaiotti.d/Documents/meansdtable")

runs100<-c(1:100,seq(50,1000,50))
allouputallruns<-rbind(alloutput100,alloutput1000)


numbers1000<-seq(150,1000,50)

allouputallruns["run"]<-runs100
numbs<-c(1:100,103:120)
alloutputallruns<-allouputallruns[numbs,]
View(alloutputallruns)

newdfmeanssd<-data.frame(matrix(vector(),120,16))

for(i in 1:length(runs100)){
  newdfmeanssd[i,1]<-alloutputallruns[i,1]
  newdfmeanssd[i,2]<-alloutputallruns[i,1]-alloutputallruns[i,2]
  newdfmeanssd[i,3]<-alloutputallruns[i,1]+alloutputallruns[i,2]
  newdfmeanssd[i,4]<-alloutputallruns[i,3]
  newdfmeanssd[i,5]<-alloutputallruns[i,3]-alloutputallruns[i,4]
  newdfmeanssd[i,6]<-alloutputallruns[i,3]+alloutputallruns[i,4] 
  newdfmeanssd[i,7]<-alloutputallruns[i,5]
  newdfmeanssd[i,8]<-alloutputallruns[i,5]-alloutputallruns[i,6]
  newdfmeanssd[i,9]<-alloutputallruns[i,5]+alloutputallruns[i,6]
  newdfmeanssd[i,10]<-alloutputallruns[i,7]
  newdfmeanssd[i,11]<-alloutputallruns[i,7]-alloutputallruns[i,8]
  newdfmeanssd[i,12]<-alloutputallruns[i,7]+alloutputallruns[i,8]
  newdfmeanssd[i,13]<-alloutputallruns[i,9]
  newdfmeanssd[i,14]<-alloutputallruns[i,9]-alloutputallruns[i,10]
  newdfmeanssd[i,15]<-alloutputallruns[i,9]+alloutputallruns[i,10]
  newdfmeanssd[i,16]<-alloutputallruns[i,11]
}

View(newdfmeanssd)

write.csv(newdfmeanssd,"C:/Users/rabaiotti.d/Documents/Ch3/newdfmeanssd.csv")

plot(newdfmeanssd[,16],newdfmeanssd[,1])

newdfmeanssd[101:118,16]<-NA
